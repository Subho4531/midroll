...
const PRIVATE_STATE_ID = 'midrollPrivateState';
...
const zkConfigPath = path.resolve(__dirname, '..', 'contracts', 'managed', 'midroll');
...
async function createProviders(walletCtx: WalletContext) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'midroll-state',
      accountId,
...
async function main() {
  ...
  const Midroll = await import(pathToFileURL(contractPath).href);
  const compiledContract = CompiledContract.make('midroll', Midroll.Contract).pipe(
